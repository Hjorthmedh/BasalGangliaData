...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "modulationDA_hd_lts", _hoc_modulationDA,
 "rate_hd_lts", _hoc_rate,
 0, 0
};
#define alpt alpt_hd_lts
#define bett bett_hd_lts
#define modulationDA modulationDA_hd_lts
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 0, "cvodeieq");
 	...
 	ivoc_help("help ?1 hd_lts hd_lts.mod\n");
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ghd = ghdbar * l * modulationDA ( _threadargs_ ) ;
   i = ghd * ( v - ehd ) ;
   }
 _current += i;
...
{
 { error =  states();
 if(error){fprintf(stderr,"at line 83 in file hd_lts.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
...
#if NMODL_TEXT
static const char* nmodl_filename = "hd_lts.mod";
static const char* nmodl_file_text = 
