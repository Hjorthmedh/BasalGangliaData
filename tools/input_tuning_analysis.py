...
def post_analysis2(network_path):
    #Load voltage trace data and validate (pass/fail) against target stated in exp_data.json 
    exp_data = json.load(open(os.path.join('BasalGangliaData','tools',"exp_data.json")))
...
def post_analysis(network_path):
    ...
    for i, v in enumerate(voltage):
        ...
        res = analyse_tuning(volt=v, num_trials=ls, mean_resting_membrane=mean_resting_membrane,
                            std_resting_membrane=std_resting_membrane)
